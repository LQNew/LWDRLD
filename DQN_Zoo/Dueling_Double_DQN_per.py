...
# And implementation of Dueling Network Architectures for Deep Reinforcement Learning (Dueling DQN)
...
class DuelingDoubleDQN_PER(object):
	def __init__(
		self,
		action_dim,
		discount=0.99, 
	...
	def train(self, replay_buffer, beta, batch_size=64):
		self.train_steps += 1

		# Sample batches from replay buffer 
		states, actions, next_states, rewards, not_dones, weights, idxes = replay_buffer.sample(batch_size, beta)
		...
		# Compute TD-error
		td_errors = np.abs((torch.squeeze(target_Q - current_Q)).cpu().data.numpy())
		new_priorities = td_errors + self.eps_per
		replay_buffer.update_priorities(idxes, new_priorities)

		# Compute Q loss
		loss = (current_Q - target_Q) ** 2
		loss = 0.5 * (weights * torch.squeeze(loss)).mean()
